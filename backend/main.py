...
#   ALLOWED_ORIGINS=https://careerpilot.vercel.app,https://www.yourdomain.com
...
# ── User accounts & saved predictions (login system) ─────────────
import json
from sqlalchemy.orm import Session
from database import engine, Base, get_db
from models_db import User, SavedPrediction
from schemas_auth import (
    SignupRequest, LoginRequest, TokenResponse, UserOut,
    SavePredictionRequest, SavedPredictionOut,
)
from auth import hash_password, verify_password, create_access_token, get_current_user

Base.metadata.create_all(bind=engine)  # creates careerpilot.db + tables on first run, no-op after
...
# ── Augment with real LinkedIn job-posting data (2024) ────────────
# Loaded from data/linkedin_dataset.csv — 1,320 real job postings from
# Kaggle's "1.3M LinkedIn Jobs & Skills (2024)" dataset, filtered down
# to our 11 career classes and capped at 120 samples per class so no
# single class dominates. If the file is missing, we just fall back
# to the 220 hand-authored samples above — the app still works.
import csv as _csv

_LINKEDIN_DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "linkedin_dataset.csv")

def _load_linkedin_dataset(path):
    rows = []
    ...
    try:
        with open(path, encoding="utf-8") as f:
            ...
            for r in reader:
                ...
                if text and career:
                    rows.append({"text": text, "career": career})
    except Exception as e:
        logger.warning(f"Could not load LinkedIn dataset at {path}: {e}")
    return rows

_linkedin_rows = _load_linkedin_dataset(_LINKEDIN_DATA_PATH)
if _linkedin_rows:
    RAW_DATASET = RAW_DATASET + _linkedin_rows
    logger.info(f"📦 Augmented dataset with {len(_linkedin_rows)} real LinkedIn job postings from {_LINKEDIN_DATA_PATH}")
else:
    logger.info("📦 No LinkedIn dataset file found — training on the 220 hand-authored samples only")

logger.info(f"📦 DATA COLLECTION: {len(RAW_DATASET)} career samples loaded")
logger.info(f"   Careers: {sorted(set(r['career'] for r in RAW_DATASET))}")
...
logger.info(f"   TF-IDF Features: {X.shape[1]} (from {len(texts)} documents)")
logger.info(f"   Ngram range: (1,2) — unigrams + bigrams")
logger.info(f"   Sample features: {tfidf.get_feature_names_out()[:10].tolist()}")
...
logger.info(f"   Trees: 200 | Max Depth: 15 | Features: {X.shape[1]}")
...
# ── Request Models ──────────────────────────────────────────────
# These constraints are enforced server-side regardless of what the
# frontend already checks — never trust client-side validation alone.
class CareerRequest(BaseModel):
    ...
    def has_input(self) -> bool:
        return bool(self.skills) or bool(self.interests)

class ResumeRequest(BaseModel):
    resume_text: str = Field(min_length=50, max_length=20000)

# ── Error handling ────────────────────────────────────────────────
# Pydantic validation failures (bad input shape/values) → clean 422 message
@app.exception_handler(RequestValidationError)
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    first_error = exc.errors()[0] if exc.errors() else {}
    field = ".".join(str(p) for p in first_error.get("loc", [])[1:])
    message = first_error.get("msg", "Invalid request")
    logger.warning(f"Validation error on {request.url.path}: {field} — {message}")
    return JSONResponse(
        status_code=422,
        content={"error": "invalid_input", "field": field, "message": message},
    )

# Anything unexpected (model/vectorizer failure, etc.) → clean 500, no stack trace leaked
@app.exception_handler(Exception)
async def unhandled_exception_handler(request: Request, exc: Exception):
    logger.error(f"Unhandled error on {request.url.path}: {exc}", exc_info=True)
    return JSONResponse(
        status_code=500,
        content={"error": "server_error", "message": "Something went wrong processing your request. Please try again."},
    )

# ── Endpoints ───────────────────────────────────────────────────
@app.get("/health")
def health():
    """Lightweight endpoint for uptime checks / deployment platforms."""
...
# ── Saved predictions (requires login) ──────────────────────────────
@app.post("/predictions/save", response_model=SavedPredictionOut, status_code=201)
def save_prediction(
...
@app.get("/predictions/history", response_model=List[SavedPredictionOut])
def get_history(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return (
        db.query(SavedPrediction)
        .filter(SavedPrediction.user_id == current_user.id)
...
def delete_prediction(
    ...
    record = (
        db.query(SavedPrediction)
        .filter(SavedPrediction.id == prediction_id, SavedPrediction.user_id == current_user.id)
        .first()
    )
    if not record:
...
def predict_career(req: CareerRequest):
    if not req.has_input():
        raise HTTPException(status_code=400, detail="Select at least one skill or interest.")
    ...
    # Seed randomness from the input itself so the same skills/interests/experience
    # always produce the same match %, instead of a different number on every call.
    rng = random.Random(hash(clean_text) ^ exp)
    ...
    for idx in top_indices:
        ...
        match = min(int(prob * 120 + rng.randint(5, 15)), 99)
        match = max(match, 35)
...
def analyze_resume(req: ResumeRequest):
    ...
    # Score calculation
    # Seeded from the resume text itself, so re-analyzing the same resume
    # gives the same score instead of a different number each time.
    rng = random.Random(hash(clean_resume))
    base_score    = min(40 + len(detected_skills) * 3 + min(word_count // 15, 30), 95)
    ats_score     = max(base_score - rng.randint(0, 8), 50)
    content_score = min(base_score + rng.randint(0, 10), 97)
