...
import { FileText, Search, BarChart3, CheckCircle2, Zap, Bookmark, Check } from 'lucide-react'
...
export default function ResumeAnalyzer() {
  ...
  const analyze = async () => {
    if (text.trim().length < 50) {
      setError('Paste at least 50 characters of resume text before analyzing.')
      return
    }
    setError(null); setLoading(true); setResult(null); setSaved(false)
    try {
      const { data } = await api.post('/analyze-resume', { resume_text: text })
      setResult(data)
    } catch {
      setError('Could not reach the analysis service. Make sure the backend is running and try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '2rem 1rem' }}>
      <h2 style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.8rem', fontWeight: 800, marginBottom: '0.4rem' }}>
        <FileText color="#6c63ff" /> Resume Analyzer
      </h2>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #2d2d4e', borderRadius: '14px', padding: '1.5rem', marginBottom: '1rem' }}>
        ...
        />
        <p style={{ fontSize: '12px', color: '#64748b', marginTop: '6px' }}>{text.split(/\s+/).filter(Boolean).length} words</p>
      </div>
      ...
      <button
        ...
        <Search size={18} /> {loading ? 'Analyzing resume...' : 'Analyze My Resume'}
      </button>
        ...
        <div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #2d2d4e', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '1.2rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px', color: '#e2e8f0', margin: 0 }}>
                <BarChart3 size={16} /> Resume Scores
              </h3>
              {user ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.25)', borderRadius: '14px', padding: '1.5rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px', color: '#34d399', marginBottom: '1rem' }}>
            ...
            <div style={{ background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.25)', borderRadius: '14px', padding: '1.5rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px', color: '#fbbf24', marginBottom: '1rem' }}>
                <Zap size={16} /> Improve This
              </h3>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #2d2d4e', borderRadius: '14px', padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {result.detected_skills.length > 0
                ...
                : <p style={{ color: '#64748b', fontSize: '14px' }}>No recognized skills detected yet — try the sample resume to see it in action.</p>
              }
            </div>
          </div>
        </div>
